...
class ModelConv64(ModelBase):

    def __init__(self, batch_size, z_dim, channels=1, y_dim=None, is_training=True):
        super().__init__(batch_size, z_dim, y_dim, is_training)
        self.channels = channels
        self.x_image = tf.placeholder(tf.float32, [batch_size, 4096], name='x_image')
    ...
    def decoder(self, z, reuse=False):
        with tf.variable_scope('decoder') as scope:
            ...
            c_i = conv(c_i, filter_size=1, stride=1, out_channels=4096*self.k, name="dec_conv_1")
            c_i = tf.reshape(c_i, shape=[self.batch_size, 4, 4, 256*self.k])
            # Now c_i has shape batch_size x 4 x 4 x 256*k

            c_i = lrelu_tconv(c_i, 4, 2, self.batch_size, 128*self.k, name="dec_tconv_2")
            # Now c_i has shape batch_size x 8 x 8 x 128*k

            c_i = bn_lrelu_tconv(c_i, 4, 2, self.batch_size, 64*self.k, self.bn_settings,  name="dec_tconv_3")
            # Now c_i has shape batch_size x 16 x 16 x 64*k

            c_i = bn_lrelu_tconv(c_i, 4, 2, self.batch_size, 32*self.k, self.bn_settings, name="dec_tconv_4")
            # Now c_i has shape batch_size x 32 x 32 x 32*k

            c_i = bn_lrelu_tconv(c_i, 4, 2, self.batch_size, 16 * self.k, self.bn_settings, name="dec_tconv_5")
            # Now c_i has shape batch_size x 64 x 64 x 16*k

            c_i = bn_lrelu_tconv(c_i, 3, 1, self.batch_size, self.channels, self.bn_settings, name="dec_tconv_fin")
            # Now c_i has shape batch_size x 64 x 64 x 1

            c_i = tf.reshape(c_i, shape=[self.batch_size, 4096])
