...
class ModelConv32(ModelBase):
    ...
    def decoder(self, z, reuse=False):
        with tf.variable_scope('decoder') as scope:
            ...
            c_i = relu_bn_tconv(c_i, 4, 2, self.batch_size, 64*self.k, self.bn_settings, name="dec_tconv_2")
            # c_i shape is batch_size x 8 x 8 x 64*k

            c_i = relu_bn_tconv(c_i, 4, 2, self.batch_size, 32*self.k,  self.bn_settings, name="dec_tconv_3")
            # c_i shape is batch_size x 16 x 16 x 32*k

            c_i = relu_bn_tconv(c_i, 4, 2, self.batch_size, 16*self.k, self.bn_settings, name="dec_tconv_4")
            # c_i shape is batch_size x 32 x 32 x 16*k

            c_i = relu_bn_tconv(c_i, 3, 1, self.batch_size, self.channels, self.bn_settings, name="dec_tconv_5")
