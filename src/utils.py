...
def conv(c_i, filter_size, stride, out_channels, name):
    in_channels = c_i.get_shape()[3]
    ...
    c_i = tf.nn.conv2d(c_i, w, strides=[1, stride, stride, 1], padding="SAME")
    c_i = c_i + b
...
def lrelu_conv(c_i, filter_size, stride, out_channels, name):
    c_i = tf.maximum(0.2 * c_i, c_i)
    c_i = conv(c_i, filter_size, stride, out_channels, name)
    return c_i


def tconv(c_i, filter_size, stride, batch_size, out_channels, name):
    in_channels = c_i.get_shape()[3]
    # Assume square
    in_size = c_i.get_shape()[1]
    ...
    b = tf.get_variable('b_%s' % name, shape=[out_channels],
                        initializer=tf.constant_initializer(0))
    c_i = tf.nn.conv2d_transpose(c_i, w, output_shape=[batch_size, out_size, out_size, out_channels],
...
def bn_lrelu_tconv(c_i, filter_size, stride, batch_size, out_channels, bn_settings, name):
    c_i = batch_norm(c_i, scope='bn_%s' % name, **bn_settings)
    c_i = tf.maximum(0.2 * c_i, c_i)
...
def lrelu_tconv(c_i, filter_size, stride, batch_size, out_channels, name):
    c_i = tf.maximum(0.2 * c_i, c_i)
    c_i = tconv(c_i, filter_size, stride, batch_size, out_channels, name)
    return c_i


def lin(c_i, size, name):
    in_size = c_i.get_shape()[1]
    w = tf.get_variable('W_%s' % name, shape=[in_size, size],
                        initializer=tf.contrib.layers.xavier_initializer())
    b = tf.get_variable('b_%s' % name, shape=[size],
                        initializer=tf.constant_initializer())
    c_i = tf.matmul(c_i, w) + b
    return c_i


def lin_bn_lrelu(c_i, size, bn_settings, name):
    c_i = lin(c_i, size, name)
    c_i = batch_norm(c_i, scope='b_n_%s' % name, **bn_settings)
    c_i = tf.maximum(0.2 * c_i, c_i)
    return c_i


def lin_lrelu(c_i, size, name):
    c_i = lin(c_i, size, name)
    c_i = tf.maximum(0.2 * c_i, c_i)
    return c_i
...
#     for i in range(1, n):
#         l_i = c_i
#         c_i = lrelu_bn_conv(c_i, 3, 1, fn, bn_settings, name="%s_blockA%d" % (name, i))
#         c_i = lrelu_bn_conv(c_i, 3, 1, fn, bn_settings, name="%s_blockB%d" % (name, i))
#         c_i = c_i + l_i
#
#     l_i = c_i
#     c_i = lrelu_bn_conv(c_i, 3, 1, fn, bn_settings, name="%s_blockA_f" % name)
#
#     # Decrease dimension
#     if reduce:
#         c_i = lrelu_bn_conv(c_i, 4, 2, fn * 2, bn_settings, name="%s_blockB_f" % name)
#         l_i = conv(l_i, 4, 2, fn * 2, name="%s_proj" % name)
#     else:
#         c_i = lrelu_bn_conv(c_i, 3, 1, fn, bn_settings, name="%s_blockB_f" % name)
#
#     c_i = c_i + l_i
#
#     return c_i
...
#     # Increase dimension
#     c_i = lrelu_bn_conv(c_i, 3, 1, fn, bn_settings, name="%s_blockA_f" % name)
#     c_i = lrelu_bn_tconv(c_i, 4, 2, batch_size, fn // 2, bn_settings, name="%s_blockB_f" % name)
#
...
#     # Increase dimension
#     c_i = lrelu_bn_conv(c_i, 3, 1, fn, bn_settings, name="%s_blockA_f" % name)
#     c_i = lrelu_bn_conv(c_i, 3, 1, fn * 2, bn_settings, name="%s_blockB_f" % name)
#
