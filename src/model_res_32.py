...
#   Uses residual architecture
class ModelRes32(ModelBase):
    ...
    def decoder(self, z, reuse=False):
        with tf.variable_scope('decoder') as scope:
            ...
            c_i = block_res_tconv(c_i, 128*self.k, self.n, self.batch_size, self.bn_settings, name="dec_b1")
            # c_i shape is batch_size x 8 x 8 x 64*k

            c_i = block_res_tconv(c_i, 64*self.k, self.n, self.batch_size, self.bn_settings, name="dec_b2")
            # c_i shape is batch_size x 16 x 16 x 32*k

            c_i = block_res_tconv(c_i, 32*self.k, self.n, self.batch_size, self.bn_settings, name="dec_b3")
    ...
    def gan(self, x_image, reuse, discriminator):
        ...
        with tf.variable_scope('gan') as scope:
            ...
            # Now c_i has shape bs x 1 x 1 x 1
