...
from src.utils import bn_lrelu_conv, lrelu_conv
...
class ModelSubpix128(ModelBase):
    ...
    def encoder(self):
        ...
        # c_i shape is batch_size x 128 x 128 x 16
        ...
        c_i = bn_lrelu_conv(c_i, 4, 2, 256*self.k, self.bn_settings, name="enc_conv_5")
        # c_i shape is batch_size x 8 x 8 x 256*k

        c_i = bn_lrelu_conv(c_i, 4, 2, 512*self.k, self.bn_settings, name="enc_conv_6")
        # c_i shape is batch_size x 4 x 4 x 512*k

        c_i = bn_lrelu_conv(c_i, 4, 4, self.z_dim, self.bn_settings, name="enc_conv_7")
    ...
    def decoder(self, z, reuse=False):
        with tf.variable_scope('decoder') as scope:
            ...
            c_i = bn_lrelu_conv(c_i, 3, 1, 1024 * self.k, self.bn_settings, name="dec_conv_2")
            c_i = PS(c_i, 2, 256 * self.k)
            # c_i shape is batch_size x 8 x 8 x 256*k

            c_i = bn_lrelu_conv(c_i, 3, 1, 512*self.k, self.bn_settings, name="dec_conv_3")
            c_i = PS(c_i, 2, 128*self.k)
            # c_i shape is batch_size x 16 x 16 x 128*k

            c_i = bn_lrelu_conv(c_i, 3, 1, 256*self.k, self.bn_settings, name="dec_conv_4")
            c_i = PS(c_i, 2, 64*self.k)
            # c_i shape is batch_size x 32 x 32 x 64*k

            c_i = bn_lrelu_conv(c_i, 3, 1, 128 * self.k, self.bn_settings, name="dec_conv_5")
            c_i = PS(c_i, 2, 32 * self.k)
            # c_i shape is batch_size x 64 x 64 x 32*k

            c_i = bn_lrelu_conv(c_i, 3, 1, 64, self.bn_settings, name="dec_conv_6")
            c_i = PS(c_i, 2, 16)
            # c_i shape is batch_size x 128 x 128 x 16
    ...
    def gan(self, x_image, reuse=False, features=False):
        with tf.variable_scope('gan') as scope:
            ...
            c_i = bn_lrelu_conv(c_i, 4, 2, 256*self.k, self.bn_settings, name="gan_conv_5")
            # c_i shape is batch_size x 8 x 8 x 256*k

            c_i = bn_lrelu_conv(c_i, 4, 2, 512*self.k, self.bn_settings, name="gan_conv_6")
            # c_i shape is batch_size x 4 x 4 x 512*k

            c_i = bn_lrelu_conv(c_i, 4, 4, 1, self.bn_settings, name="gan_conv_7")
