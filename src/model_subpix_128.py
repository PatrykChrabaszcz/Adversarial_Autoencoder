...
class ModelSubpix128(ModelBase):
    ...
    def encoder(self):
        ...
        c_i = relu_bn_conv(c_i, 4, 2, 256*self.k, self.bn_settings, name="enc_conv_5")
        # c_i shape is batch_size x 8 x 8 x 256*k

        c_i = relu_bn_conv(c_i, 4, 2, 512*self.k, self.bn_settings, name="enc_conv_6")
    ...
    def decoder(self, z, reuse=False):
        with tf.variable_scope('decoder') as scope:
            ...
            c_i = conv(c_i, 1, 1, 4096*self.k, name="dec_conv_1")
            c_i = PS(c_i, 2, 1024*self.k)
            # c_i shape is batch_size x 2 x 2 x 1024*k

            c_i = relu_bn_conv(c_i, 3, 1, 2048 * self.k, self.bn_settings, name="dec_conv_2")
            c_i = PS(c_i, 2, 512 * self.k)
            # c_i shape is batch_size x 4 x 4 x 512*k

            c_i = relu_bn_conv(c_i, 3, 1, 1024 * self.k, self.bn_settings, name="dec_conv_3")
            c_i = PS(c_i, 2, 256 * self.k)
            # c_i shape is batch_size x 48 x 8 x 256*k
