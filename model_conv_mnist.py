...
class ModelConvMnist:
    def __init__(self, batch_size, z_dim):
        ...
        self.filter_numbers = [16, 32,  z_dim]
        self.strides = [1, 2, 1]
        self.filter_sizes = [3, 4, 12]
        ...
        n_input = 28 * 28
        # Input image
        self.x_image = tf.placeholder(tf.float32, [batch_size, n_input], name='x_image')
    ...
    def decoder(self, z):
        ...
        for i, shape in enumerate(self.data_shapes):
            ...
            b = tf.get_variable("b_dec_tconv%d" % i, shape=self.filter_shapes[i][2],
                                initializer=tf.constant_initializer(0))
            s = self.strides[i]
            output = lrelu(tf.nn.conv2d_transpose(current_input, w,
                                                  #tf.pack([self.batch_size, shape[1], shape[2], shape[3]]),
                                                  shape,
                                                  strides=[1, s, s, 1], padding='VALID') + b)
            current_input = output

        y_image = current_input
        y_image = tf.reshape(y_image, shape=[-1, self.input_dim])
